  ...
  while (!m_globalFinished)
    {
      ...
      if (nextTime > m_grantedTime || IsLocalFinished () )
        ...
        /* updating the clock of each fncsapplication*/
        // Locate the FncsApplication instances with the same names.
         //       Ptr<FncsApplication> app1 =
          //        Names::Find<FncsApplication>("fncs_H_S_1");
          //      Ptr<FncsApplication> app2 =
          //        Names::Find<FncsApplication>("fncs_BrainH_1");
           //     if (!app1) {
           //       NS_FATAL_ERROR("failed FncsApplication lookup for current simulation time update'");
            //    }
            //    if (!app2) {
         //         NS_FATAL_ERROR("failed FncsApplication lookup to for current simulation time update'");
           //     }
             //   app1->SetCurrentSimulationTimeInApplicationObject(m_currentTs);
           //     app2->SetCurrentSimulationTimeInApplicationObject(m_currentTs);
        /* */

          if (m_grantedTime == GetMaximumSimulationTime ().GetTimeStep()) {
            Stop();
          }
          else {
